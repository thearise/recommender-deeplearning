...
class RBMAlgorithm(AlgoBase):
    ...
    def fitById(self, trainset, testUser=85):
        ...
        print('training matrix 01 ', trainingMatrix.shape)
        ...
        print("Retrieving user011 ", testUser, "Inner id: ", innerTestId, ' & ', len(shaping[0]))
    ...
    def fitByIdRec(self, trainset, testUser=85):
        AlgoBase.fit(self, trainset)
        ...
        recs = rbm.GetRecommendationsAtCkPt([trainingMatrix[innerTestId]])
        recs = np.reshape(recs, [numItems, 10])
        predic = np.zeros([numUsers, numItems], dtype=np.float32)
            # np.savetxt("testingcsv2.csv", itemS, delimiter=",")
        for itemID, rec in enumerate(recs):
